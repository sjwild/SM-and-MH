...
dag_covariates_unobserved_u_w %>% 
  tidy_dagitty() %>% 
  mutate(colour = ifelse(name %in% c("U1", "U2", "W1", "W2"), 
                         "grey50", "black")) %>% 
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
    geom_dag_point(mapping = aes(colour = colour), show.legend = FALSE) +
    geom_dag_edges(mapping = aes(edge_colour = colour)) +
    geom_dag_text() + scale_color_manual(values = c("black", "grey50")) +
    theme_dag()
```
...
dag_covariates_unobserved_u_w_z %>% 
  tidy_dagitty() %>% 
  mutate(colour = ifelse(name %in% c("U1", "U2", "W1", "W2"), 
                         "grey50", "black")) %>% 
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
...
dag_line_z_MH2 <- dagitty("dag{
                          Z -> MH2;
                          }")

coordinates(dag_covariates_unobserved_u_w_z) <- DAG_coords
coordinates(dag_line_z_MH2) <- DAG_coords
...
dag_line_z_MH2 %>%
  geom_dag_edges_arc(mapping = aes(x = x, y = y, xend = xend, yend = yend),
                     edge_colour = "red") -> p2
