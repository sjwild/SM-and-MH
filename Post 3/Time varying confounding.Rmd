...
DAG_coords <- list(x = c(MH1 = 1, MH2 = 2,
                         X1 = 1.25, X2 = 2.25,
                         SM1 = 1, SM2 = 2,
                         U1 = 0.75, U2 = 1.75,
                         Ui = 1.5,
...
* **colliders**: When a variable is caused by two others. We want to exclude them from our regression, because they can open backdoor paths.  
...
```{r confounding, collider, mediator}
# confounding, collider, mediation

confounding <- dagitty("dag{
                       X <- U -> Y;
                       }")
coordinates(confounding) <- list(x = c(X = 0, Y = 1, U = 0.5),
                                 y = c(X = 0, Y = 0, U = 0.5))

p_confounding <- ggdag(confounding) + lims(y = c(-1, 1))

collider <- dagitty("dag{
                       X -> C <- Y;
                       }")
coordinates(collider) <- list(x = c(X = 0, Y = 1, C = 0.5),
                                 y = c(X = 0, Y = 0, C = 0.5))

p_collider <- ggdag(collider) + lims(y = c(-1, 1))


mediator <- dagitty("dag{
                       X -> M -> Y;
                       }")
coordinates(mediator) <- list(x = c(X = 0, Y = 1, M = .5),
                                 y = c(X = 0, Y = 0, M = 0))

p_mediator <- ggdag(mediator) + lims(y = c(-1, 1))


p_confounding + p_collider + p_mediator + plot_layout(ncol = 1)
```
...
Look at the image above. in the top DAG, we have an arrow going from U to X and from U to Y. There is no arrow from X to Y. This means that X and Y will appear associated unless we control for U. Sometimes it's fine--we can see U and control for it. But sometimes it isn't, especially when U is unobserved. And U is often unobserved. The key message here is that we want to control for confounding.

The middle DAG shows a collider. Here, X and Y each have an arrow into C. There is no arrow from X to Y. In this scenario, we do not want to control for C. Why? An example helps. The standard example is beauty and talent in hollywood actors and actresses. In the general population, acting talent and beauty are unrelated. That is, there is no arrow from X (talent) to Y (beauty) or vice versa. But to succeed in hollywood, an actor or actress needs to talented or beautiful. Talented actors and actresses do not need to be beautiful, and beautiful actors and actresses do not need to be talented. If we look only at hollywood, we would think that individuals are either talented or beautiful, but not both. The key message about collidrs is that we **do not** want to control for them.

The bottom DAG shows a mediator. We have an arrow going from X to M, and another form M to Y. There is no arrow that goe directly from X to Y. Here, our causal relationship is mediated by M. If we control for M, we interrupt the path from X to Y, and it will look like X does not cause Y. Sometime we do not want to control for mediators, but sometimes we do. It's complicated.
...
* SM1 and SM2 represent social media use at time 1 and time 2. SM2 is our exposure.
* MH1 and MH2 are mental health at time 1 and time 2. MH2 is our outcome variable.
...
* W1 and W2 are unobserved variables at time 1 and time 2 and are directly related only to ssocial media use and mental health (but not observerd covariates). W1 and W2 are how social media can affect an indiviudal even if they are not using social media. An example of this type of unobserved variable could include a new social media platform launching that cause someone to switch platforms, changes to an existing platform's algorithm that affect use and mental health, or changes in how your friends use social media, which in turn affects your mental health and how you use social media. 
...
To start with, we have social media (SM) at time 1 and time 2--like, for instance, in two waves of a survey, or when entering and completing a social media reduction experiment. SM affects our mental health, MH. There are only arrows from SM to MH at each time point. There are no other paths to be concerned about here: no confounding, no colliders, no mediators.

```{r First dag}
first_dag <- dagitty("dag{
    SM1 -> MH1;
    SM2 -> MH2;
    SM2 [exposure];
    MH2 [outcome];
...
coordinates(first_dag) <- DAG_coords

ggdag(first_dag)

```

But of course this DAG is a poor representation of the real world. In the real world, we would expect social media use in t1 to influence use in t2. There's lots of reasons why. For instance, if I use social media more today, maybe I need to use it more tomorrow to respond to all the messages I received. Or maybe I use it lots today so I use it les tomorrow because I'm bored of it and need a break. Same for mental health. Our mental health today probably influences our mental health tomorrow, and it probably influences our social media use tomorrow as well. So let's add those arrows to our DAG.
...
coordinates(simple_dag) <- DAG_coords

ggdag(simple_dag)

```

Remember our paths up above. If we look at our simple DAG we see that there is confounding: mental health at time 1 (MH1) affects our mental health at time 2 **AND** our social media use at time 2. So we need to control for mental health at time 1 if we want to estimate the causal effect of our exposure, social media use, at time 2.

Now we can start to see why DAGs are useful. Without thinking through our assumptions, I wouldn't have known to control for mental health at time 1. And as a result, I would fail to recover the causal effect of social media at t2 on my mental health at t2. 

## A more complicted DAG with covariates
Our DAG, though simple, is still missing a few things. There are things we measure, like age, that likely affect social media use. We need to make sure we include those. things like age likely affect both social media and mental health, so we'll draw arrows from our covariates to social media and mental health. What matters here for our purposes is that these are **measured** covariates--that is, we know their values. Common measured covariates include things like age, sex or gender, household income, and so on. For many--if not all--of these covariates, it is unlikely that social media causes them, so we **won't** draw an arrow from social media to our covariates.

```{r simple DAG with covariates}
# draw second dag
simple_dag_covariates <- dagitty("dag{
    SM1 -> MH1 -> SM2;
    SM1 -> SM2;
    SM2 -> MH2;
    MH1 -> SM2;
...
coordinates(simple_dag_covariates) <- DAG_coords

ggdag(simple_dag_covariates)

```

Now we're starting to get to something more realistic. Our DAG features a treatment, our exposure (social media use at time 2), our outcome (mental health at time 2), and covariates that affect our mental health and social media use. Even though it is more complicated that the simpler one we presented above, we can sstill recover our causal effect if we control for mental health at time 1 and our covariates at time 2. That is, we need to control for MH1 and X2. Although there are a bunch of arrows, including these two variables allow us to close all the backdoor paths.
...
The first sort of unobserved is a time-invariant variable. This is something that is constant across time (hence invariant). As you can see from the DAG below, it affects everything. While it looks scary here, we can adjust for it in a model by including something like fixed effects or a random intercept. You can see them in the DAG below, represented by Ui. 

```{r DAG with covariates and Ui}
dag_covariates_unobserved <- dagitty("dag{
    SM1 -> MH1 -> SM2;
    SM1 -> SM2;
    SM2 -> MH2;
    MH1 -> SM2;
...
coordinates(dag_covariates_unobserved) <- DAG_coords

ggdag(dag_covariates_unobserved)
```

The second sort is much harder to control for. These are time-varying unobserved variables. You can see them below represented by U1, U2, W1 and W2. And it's these variables that are going to cause the most trouble. So much trouble, in fact, that it becomes impossible (at least so far as I know) to recover an unbiased causal effect.

```{r dag with unobserved time-varying confounding}

dag_covariates_unobserved_u_w <- dagitty("dag{
    SM1 -> MH1 -> SM2;
    SM1 -> SM2;
    SM2 -> MH2;
    MH1 -> SM2;
...
coordinates(dag_covariates_unobserved_u_w) <- DAG_coords

ggdag(dag_covariates_unobserved_u_w)

```

These variables--U1, U2, W1 and W2--lead to **time-varying confounding**. Time-varying confounding occurs when we have an unobserved variable that affects both our treatment (social media use) and our menetal health, and that unobserved variable changes over time. Take how your friends use social media. If they use social media more, you use it more. Because you use it more, your social media use also affects your mental health. And because you friends are on social media so much, they can't hang out with you. This also affects your mental health, but through a path other than **your** social media use. Unless you can control for that, you have no way of attributing the variation in your mental health to your social media use, to your friends, or some combination thereof. It all gets jumbled together. 
...
U1 and U2 and W1 and W2 imply a violation of parallel trends and lack of sequential exchangability, which means that many of our usual approaches may not work well. How well they could work depends on the severity of the violations.
...
A second potential way to address this is to restrict our longitudinal analysis to users who exclusively use one specific social media platform (say, only TikTok), or who meet other prescribed criteria. While this can work, the risk is we shrink our sample size down to an unworkable level. We also have the risk that our causal effects may not be transportable or generalizable--that is, they may not apply to other social media sites, or to other users of the same site, and so on.
...
In the DAG below, you can see our instrument, Z, with an arrow heading into X2. There are no other arrows from Z to MH2, and no arrows heading into Z. And we know there are no arrows heading into Z because Z is randomly assigned by the researchers.

```{r DAG with IV}


dag_covariates_unobserved_u_w_z <- dagitty("dag{
    SM1 -> MH1 -> SM2;
    SM1 -> SM2;
    SM2 -> MH2;
    MH1 -> SM2;
...
coordinates(dag_covariates_unobserved_u_w_z) <- DAG_coords

ggdag(dag_covariates_unobserved_u_w_z)

```

### Caution is always warranted
In social media reduction experiments, however, we need to be careful with our instrumental variable. The reason here is subtle: our instrumental variable may influence mental health directly in addition to doing so through social media use. How? If I am participating in one of these trials, the treatment isn't blinded--that is, I know I am reducing my social media use. That knowledge may influence my mental health even if my social media use did not decrease.
...
Putting it all together, I'm brought to a somewhat gloomy conclusion: estimating the total causal effect of social media use is probably impossible given current techniques. That doesn't mean we should give up--after all, there is something to be said for the [shoe leather approach](https://www.jstor.org/stable/270939?casa_token=m04CuR0Ll8MAAAAA%3AaDMfVwnVZIacppHuxg-93IwWcUo20B0CTf8V-ujsqnQAnv0CAFUcLtFSXgqWufHAxWWCLCWm8CWOXtKMSGdjkFakPtfeeUbf30J3jpp6jD01GXJi6Uus). But it does mean I think we need to be careful with our language and tenative in our conclusions until better techniques come along. There is a lot of work trying to address problems like time-varying confounding, interference, and other issues that arise in social networks. Let's see where it goes.

# Thank you, references, and other reading
I owe a big thank you to X,Y, and Z for reviewing this post and catching all my errors. All the good stuff belongs to others, the bad stuff is me.
